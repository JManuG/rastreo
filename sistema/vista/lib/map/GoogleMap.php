...
function getCache($address) {
       $_ret = array();
        ...
        $_res =mysql_query("SELECT lon,lat FROM GEOCODES  where
address ='".$address."'");

        if($_row =mysql_fetch_array($_res)) {
            $_ret['lon'] = $_row[0];
            $_ret['lat'] = $_row[1];
        }
...
function putCache($address, $lon, $lat) {
       // PEAR DB
        ...
        $_res =mysql_query("insert into '.$this->_db_cache_table.'
values ('".$address."', '".$lon."', '".$lat."')");
